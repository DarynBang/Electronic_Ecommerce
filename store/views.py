...
def store(request):
    ...
    laptops = Laptop.objects.select_related('id').filter(id__status='New').order_by('?')[:3]
    keyboards = Keyboard.objects.select_related('id').filter(id__status='New').order_by('?')[:3]
    mice = Mouse.objects.select_related('id').filter(id__status='New').order_by('?')[:3]
    earphones = Headphone.objects.select_related('id').filter(id__status='New').order_by('?')[:3]
...
def laptop_page(request):
    ...
    laptops = (
        Laptop.objects
        .select_related('id')  # Include related Product data for efficiency
...
def updateItem(request):
    ...
    order, created = Transaction.objects.get_or_create(id=customer_id, status="Pending", defaults={"total": 0})

    orderItem, created = OrderItem.objects.get_or_create(order=order, product=product, defaults={"previous_status": product.status})
